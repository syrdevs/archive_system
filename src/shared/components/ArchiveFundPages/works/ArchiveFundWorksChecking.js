...
import {isEmpty, differenceWith, isEqual, pickBy} from 'lodash';
import AntModal from '../../AntModal';
import {addDerivativeWorks, getValuesOfObjsWithProps, updateCubeData} from '../../../actions/actions';
      ...
      .then(res => {
        ...
        if(res.success) {
          ...
          const datas = mode === 'temp' ? [{
            ...
            own: [{doConst: DO_FOR_WORKS, doItem: this.props.location.state.workId, isRel: "0", objData: {} }],
            props: [
              {propConst: 'workStatusAvailabilityCheck', val: String(this.props.tofiConstants.completed.id), typeProp: '11', periodDepend: '2', isUniq: '1'},
        ...
        <Table
          columns={[
              ...
              children: [
                {
                  ...
                  className: 'td-center',
                  render: (text, record) => (
                    <Checkbox checked={text} disabled={record.temporaryUse} onChange={e => this.onChange(e.target.checked, record.key, 'disinfection')}/>
                  )
                },
                {
                  key: 'disinfestation',
                  ...
                  className: 'td-center',
                  render: (text, record) => (
                    <Checkbox checked={text} disabled={record.temporaryUse} onChange={e => this.onChange(e.target.checked, record.key, 'disinfestation')}/>
                  )
                },
                {
                  key: 'restoration',
                  ...
                  className: 'td-center',
                  render: (text, record) => (
                    <Checkbox checked={text} disabled={record.temporaryUse} onChange={e => this.onChange(e.target.checked, record.key, 'restoration')}/>
                  )
                },
                {
                  key: 'binding',
                  ...
                  className: 'td-center',
                  render: (text, record) => (
                    <Checkbox checked={text} disabled={record.temporaryUse} onChange={e => this.onChange(e.target.checked, record.key, 'binding')}/>
                  )
                },
                {
                  ...
                  className: 'td-center',
                  render: (text, record) => (
                    <Checkbox checked={text} disabled={record.temporaryUse} onChange={e => this.onChange(e.target.checked, record.key, 'restorationOfFadingTexts')}/>
                  )
                },
              ]
            },
            {
              ...
              className: 'td-center',
              render: (text, record) => (
                <Checkbox checked={text} disabled={record.temporaryUse} onChange={e => this.onChange(e.target.checked, record.key, 'irreparablyDamaged')}/>
              )
            }
          ]}
          bordered
